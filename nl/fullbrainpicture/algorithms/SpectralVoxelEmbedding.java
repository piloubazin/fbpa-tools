...
public class SpectralVoxelEmbedding {
	...
	public final void voxelDistanceReferenceSparseEmbedding(int depth, double alpha) {
        ...
        for (int dim=1;dim<ndims+1;dim++) {
            ...
            norm = FastMath.sqrt(norm);
            if (normalize) for (int xyz=0;nxyz<nxyzr;xyz++) {
        ...
        for (int v1=0;v1<ndims-1;v1++) for (int v2=v1+1;v2<ndims;v2++) {
            double prod=0.0;
            for (int xyz=0;nxyz<nxyzr;xyz++) prod += imgEmbedding[xyz+v1*nxyzr]*imgEmbedding[xyz+v2*nxyzr];
            mean += prod;
            num++;
            if (prod>max) max = prod;
            if (prod<min) min = prod;
