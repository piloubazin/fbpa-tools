...
public class SpectralMeshEmbeddingDebug {
	...
	public void meshDebugDistanceEmbedding(int depth, float threshold) {
	...
	public final void meshDistanceSparseEmbedding(int depth) {
        ...
        for (int n=0;n<msize*step;n+=step) {
            for (int d=0;d<depth;d++) {
	            int m = (closest[d][n]-1)*step;
	            if (m>=0) {
	                matrix[n/step][m/step] = 1.0/(1.0+distances[d][n]);
	                matrix[m/step][n/step] = 1.0/(1.0+distances[d][n]);
	            }
	        }
	    }
        ...
        embeddingList = new float[npt*ndims];
        for (int dim=0;dim<ndims;dim++) {
            System.out.println("eigenvalue "+(dim+1)+": "+eigval[dim]);
            for (int n=0;n<npt;n++) {
                embeddingList[n+dim*npt] = (float)eig.getV().getEntry(closest[0][n]-1,dim);
            }
        }
        ...
        if (eigenGame && step>1) {
            ...
            for (int n=0;n<npt;n++) {
                for (int m=0;m<ngbp[n].length;m++) {
                    if (ngbp[n][m]>n) {
                        int ngb = ngbp[n][m];
                        double dist = Numerics.square(pointList[3*n+X]-pointList[3*ngb+X])
                                     +Numerics.square(pointList[3*n+Y]-pointList[3*ngb+Y])
                                     +Numerics.square(pointList[3*n+Z]-pointList[3*ngb+Z]);
                        double coeff = 1.0/(1.0+FastMath.sqrt(dist));
                        mtxval[id] = coeff;
                        mtxid1[id] = n;
                        mtxid2[id] = ngb; 
            ...
            double[][] init = new double[ndims][vol];
            double[] norm = new double[ndims];
            for (int dim=0;dim<ndims;dim++) {
                for (int n=0;n<npt;n++) {
                    init[dim][n] = embeddingList[n+dim*npt];
            ...
            runSparseLaplacianEigenGame(mtxval, mtxid1, mtxid2, mtxinv, nmtx, vol, ndims, init, nconnect, alpha);
                              
            for (int dim=0;dim<ndims;dim++) {
                for (int n=0;n<npt;n++) {
                    embeddingList[n+dim*npt] = (float)init[dim][n];
                }
            }
        }            
		return;
