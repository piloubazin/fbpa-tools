...
public class Gdm3d {
	...
	private 	float[] 		target;  		    // target levelset (e.g. to approximate or smooth)
	...
	// parameters
	private	double		smoothweight, fieldweight, balloonweight, targetweight;
	...
	public Gdm3d(int[] init_, int nx_, int ny_, int nz_,
						...
						float[][] field_, float[] balloon_, float[] target_,
						float fw_, float bw_, float sw_, float tw_,
						String connectivityType_) {
		this(init_, nx_, ny_, nz_, rx_, ry_, rz_, field_, balloon_, target_, fw_, bw_, sw_, tw_, connectivityType_, null);
	}
	public Gdm3d(int[] init_, int nx_, int ny_, int nz_,
						float rx_, float ry_, float rz_,
						float[][] field_, float[] balloon_, float[] target_,
		...
		if (debug) System.out.print("GDM forces: "+fw_+" (field), "+bw_+" (balloon), "+tw_+" (target), "+sw_+" (smoothing)\n");
	...
	public Gdm3d(float[] lvl_, float maxdist_, int nx_, int ny_, int nz_,
						...
						float fw_, float bw_, float sw_, float tw_,
						String connectivityType_) {
		this(lvl_, maxdist_, nx_, ny_, nz_, rx_, ry_, rz_, field_, balloon_, target_, fw_, bw_, sw_, tw_, connectivityType_, null);
	}	
	public Gdm3d(float[] lvl_, float maxdist_, int nx_, int ny_, int nz_,
						float rx_, float ry_, float rz_,
						float[][] field_, float[] balloon_, float[] target_,
    ...
    */
    ...
    private final double levelsetForces(int xyz) {
		...
		// target forces, depend on distances
		double delta = 0.0;
		if (Numerics.abs(targetweight)>0) {
			double DeltaP = Math.sqrt(Numerics.square(Numerics.max(Dmx, 0.0)) + Numerics.square(Numerics.min(Dpx, 0.0))
									 +Numerics.square(Numerics.max(Dmy, 0.0)) + Numerics.square(Numerics.min(Dpy, 0.0))
									 +Numerics.square(Numerics.max(Dmz, 0.0)) + Numerics.square(Numerics.min(Dpz, 0.0)));
			...
			double deltaforce = - Numerics.bounded( target[xyz] - levelset[xyz], -1.0, 1.0);
			
			delta = targetweight*stepsize*(Numerics.max(deltaforce, 0.0)*DeltaP + Numerics.min(deltaforce, 0.0)*DeltaM);
		}		
		
		return - (smooth - balloon - field - delta);
    }
