...
public class Mgdm3d {
	...
	private 	int[] 		    balloonlabels;  	// original image forces, along object normals (e.g. from memberships)
	...
	public Mgdm3d(int[] init_, int nx_, int ny_, int nz_, int nobj_, int nmgdm_,
		...
		init(init_, nx_, ny_, nz_, nobj_, nmgdm_, rx_, ry_, rz_, field_, balloon_,  labels_, fw_, bw_, sw_, pw_, connectivityType_, lutdir_);
	}
    ...
    */
    ...
    private final double levelsetForces(int xyz, int lb) {
		...
		if (Numerics.abs(balloonweight)>0) {
			...
			double bforce = 0.0;
			if (lb==balloonlabels[xyz]) {
			    bforce = Numerics.bounded(balloonforces[xyz], -1.0f, 1.0f);
			} else {
			    bforce = Numerics.bounded(-balloonforces[xyz], -1.0f, 1.0f);
            }
